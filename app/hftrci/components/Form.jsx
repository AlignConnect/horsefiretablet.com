...
const Form = () => {
    ...
    const handleFormSubmit = async (e) => {
        ...
        try {
            ...
            formdata.append("post", "Horsefiretablet.com/HFTRCI");
            formdata.append("ip", check ? check : "");
            formdata.append("checkout_toogle", checkoutDetail.checkoutChecked);
            formdata.append("blog_url", process.env.NEXT_APP_URL_HFTRCI ?? "horsefiretablet.com/hftrci");
    ...
    return (
        <>
            ...
            <section className='bg-white '>
                ...
                <div className="relative z-20 pt-0 pt-5 pb-10 sm:py-10 px-3 md:px-10" id="form">

                    <div className="text-center ">
                        <h1 className="text-[1.7rem] md:text-4xl font-bold  ">कॉल पर विशेषज्ञ डॉक्टरों से मुफ्त<br />  सलाह के लिए फॉर्म भरें!</h1>
                    </div>
                    <div className="text-white relative mt-2 sm:mt-5 " id='formcallhide' >
                        <div className="relative z-10 max-w-2xl mx-2 md:mx-12 md:px-4 px-1 rounded-md" style={{ boxShadow: "rgba(0, 0, 0, 0.24) 0px 3px 8px" }}>
                            <div className="py-5">
                                <div className="text-center">
                                    ...
                                    {CustomerDataValue?.map((data, key) => {
                                        return <div className="flex flex-col" key={key}>
                                            ...
                                            <div className="relative z-20">
                                                <input {...data.inputValue} className={`px-3 py-3 border border-black text-black w-full rounded-lg bg-white  ${data.icon ? 'pl-[4.5rem]' : ''}`} onChange={handleChange} value={CustomerData[data.inputValue.name]} />
                                                {data.icon && <span className="absolute fontPoppins text-black ransform translate-x-[-50%] translate-y-[-50%] top-1/2 left-7 border-r border-black px-2">{data.icon}</span>}
                                            </div>
                                        </div>
                                    })}

                                    <div className="flex justify-center items-center pt-6 ">
                                        <button onClick={handleFormSubmit} type="button" className="z-20 text-black fontPoppins text-2xl font-bold px-3 py-3  md:w-72 w-full rounded-md bg-[#f8bd21] ">
                                            SUBMIT
                                        </button>
                                    </div>
                                </form>
                        ...
                        <Image fill alt='hft' loading='lazy' src='https://imagedelivery.net/aacnHGAqlUDhaplA3bnkbA/48f51f8a-0c1b-4427-7f46-3995516b2500/public' className='rounded-3xl' />
