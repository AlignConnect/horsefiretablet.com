...
const Formdata = () => {
    ...
    const handleFormSubmit = async (e) => {
        ...
        try {
            ...
            formdata.append("post", "Horsefiretablet.com/HFTAB")
            formdata.append("ip", check ? check : "")
            formdata.append("checkout_toogle", checkoutDetail.checkoutChecked)
            formdata.append("blog_url", process.env.NEXT_APP_URL_HFTAB ?? "horsefiretablet.com/hftab")
    ...
    return (<div className={`${noto.className} py-5`} id="form">
        ...
        <div>
            <div className="max-w-2xl mx-auto px-3" id='form'>
                ...
                <h1 className={`${noto.className} text-3xl text-center mb-2 md:my-6 text-black font-bold`}>कॉल पर विशेषज्ञ डॉक्टरों से मुफ्त सलाह के लिए फॉर्म भरें !</h1>
                <div className="" id='formcallhide'>
                    <div className="mx-auto max-w-5xl bg-gray-100 px-4 py-6 border border-black rounded-lg ">
                        <div>
                                ...
                                {CustomerDataValue?.map((data, key) => {
                                    return <div className="flex flex-col my-6" key={key}>
                                        ...
                                        <div className="relative">
                                            ...
                                            {data.icon && <span className={`${noto.className} absolute transform translate-x-[-50%] translate-y-[-50%] top-1/2 left-5 border-r border-black px-2`}>{data.icon}</span>}
                                        </div>
                                    </div>
                                })}
                                <div className="flex justify-center items-center pt-4">
                                    <button onClick={handleFormSubmit} type="button" className="bg-[#ebc73b] text-xl px-3 pt-3 pb-1 md:w-72 w-full rounded-full hover:bg-black hover:text-white transition-colors font-bold">
                                        SUBMIT
                                    </button>
                                </div>
